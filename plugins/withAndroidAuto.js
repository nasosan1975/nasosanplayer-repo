...
var RNTP_ADAPTER_KT = [
    ...
    "        val playlist: ImmutableList<MediaItemData> = if (title != null) {",
    "            ImmutableList.of(",
    ...
    "            stateBuilder.setPlaylist(playlist, 0, 0L)",
    "        }",
    ...
    "            Player.COMMAND_SEEK_TO_MEDIA_ITEM -> rntp?.skip(mediaItemIndex)",
    "            else -> rntp?.seekTo((positionMs / 1000.0).toFloat())",
    "        }",
    "        invalidateState()",
    "        return Futures.immediateVoidFuture()",
    "    }",
...
var BROWSER_SERVICE_KT = [
    ...
    "            val idx = mediaItems.firstOrNull()?.mediaId?.toIntOrNull()",
    "            if (idx != null) {",
    "                MusicService.nasosanInstance?.skip(idx)",
    "                MusicService.nasosanInstance?.play()",
    "            }",
